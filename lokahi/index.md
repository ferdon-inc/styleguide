...
4.  After creating the Contract, you will be directed back to the dashboard and will see a green message that says ‘Create Successfully!’.  You can then navigate to the project on the dashboard or [search for the Contract](#search-on-dashboard-page), and open the Contract to begin work on the workboard.  
    For more detailed information, see [Create New Contract on the Dashboard](#create-new-contract-on-the-dashboard), and for how to set up a basic configuration see [Add Business logit to a Contract Layer](<#add-business-logit-to-a-contract-layer-(workboard)>).
...
#### Add Business Logit to A Contract Layer (Workboard)
...
2.  A screen pops up with the title "Transactions". The default filter is set to ALL in the "Filter by:" section that includes 4 options {All; Success; Pending; Incomplete}.
...
### Workboard Toolbar (Bottom Right Of Screen)

The workboard toolbar is in the bottom right of the screen, and contains the “Download Icon”, “Print/Save Icon”, "Theme Setting Icon" and the “Support User Icon”.
...
The workboard toolbar is in the bottom right of the screen, and the “Download Icon” is located at 1st place on the toolbar. You can export the Workboard of a layer to save the work at that time and download it in JSON format.
...
The workboard toolbar is in the bottom right of the screen, and the “Print/Save Icon” is located at 2nd places on the toolbar. You can print the contents of the Workboard at any time and also have the option to ‘Save as a PDF’ with the date stamp.
...
The workboard toolbar is in the bittom right of the screen and the “Theme Setting Icon” is located at 3rd place on the toolbar. You can switch the theme of the page with the following 3 themes:  Light theme, Dark theme, and Dark Blue theme.

Procedure

1. In the toolbar, click "Theme Setting Icon” 
2. Click choose one of the three theme options: Light theme, Dark theme, and Dark Blue theme.

#### Support User Icon

The workboard toolbar is in the bottom right of the screen and the “Support User Icon” is located at 4th places on the toolbar, and contains "Start Product tour" option and "What's New" option.

#### Start Product tour option:

For new users, after accessing the page the Product tour will show up so that the user can get acquainted with all the main features of the site.\\
You can review the tour in the toolbar of the workboard page.The workboard toolbar is in the bottom right of the screen and the “Support User Icon” is located at 4th places on the toolbar. Start Product Tour is one of the options in the Support User menu.

Procedure

1. Each feature will be displayed by 1 frame:\\
    \+ Case 1: Contract layer: There are 14 steps.
    The tour will display frames introducing Add components, Define hashtag, Find components, Spin Layout, Transaction, Deployment, Recommendation, Add Data, Flow Graph, and other features, Edit Component, Support Menu, Theme Setting, Print, Download.\\
    \+ Case 2. Element layer: There are 12 steps.
    The tour will display frames introducing similar to the features in the Contract layer, but minus 2 transaction features, Deployment.\\
    \+ Case 3: Logit layer: There are 11 steps.
    The tour will display frames introducing similar features in the Contract layer, but minus 3 transaction features, Deployment, FLow Graph and add Define type feature.
2. In each introduction frame will have 3 buttons.\\
    \+ Next button: You can click on the Next button to go to the frame showing the next function of the tour.\\
    \+ Back button: You can click on the Back button to go back to the previous function show frame.\\
    \+ Skip tour button: You can click on the Skip tour button to skip this tour.
3. In the final frame of the tour, you will see the Done button. Clicking the Done button means you are done with the tour.

#### What's New option

The workboard toolbar is in the bottom right of the screen and the “Support User Icon” is located at 4th places on the toolbar. What's new is one of the options in the Support user menu. 
When the new version is released, there will be changes in UI or features. Here you can see an overview of the changes in the new version.

Procedure

1. In toolbar, click "Support User Icon” 
2. Click choose “What’s New?” option.
3. The "Welcome Back" screen will pop up and you will see a "Take a Tour" button. After clicking on the "Take a Tour" button you will begin the tour.
4. The tour will list the changes in UI, feature by frame. At each frame you can click the Next button to go to the next frame until the end of the tour, or the Skip tour button to close the tour.
5. In the final frame of the tour, you will see the Done button. Clicking the Done button means you are done with the tour.
...
#### Find Component Input Frame

The workboard toolbar is in the top right of the screen , and the “Find Component Input Frame” is located in the toolbar. You can find any block on the Workboard with the “Find Component Input Frame”.

Procedure

1.  Click on the “Find component” input frame.
2.  Enter a search text in the “Find component” input frame.
...
The workboard toolbar is in the top right of the screen , and the “Spin Layout Icon” is located in the toolbar. You can create many flow graph versions of the model to check formal verification. Spin layout icon are all observable in 3 layers: Logit layer, Element layer, Contract layer.

Procedure

1. Click pn the Spin layout icon.
2. Go to the [Spin Layout](#spin-layout) to see the screen of Spin Layout.
...
1.  When you click on a block in the left table of "Configure port" screen, a drop down list appears with 3 options {Input Port, Output Port, Both Port}.
2.  After you click on a block and choose an option, in the left table of the "Configure port" screen, a frame appears that includes "Name", "Description" input frames, "Write long Description" button and "Clear Port" button, and the "Required Hashtags" input, the "add more" (+) icon button enables. At the same time, the block you have selected the port will change color: Input port will be green color, the output port will be blue color,  Both ports will be pink color.
...
4.  The "Add more" (+) icon button enables you to add more “Required Hashtag” field.
    ...
    \[Note:\\
    \+ When using the hashtags in the same "Required Hashtag" input then click "Fix it" at [Recommendation](#recommendation-feature) sidebar in the higher layer, it shows all the blocks relate to those hashtags and just choose one block in the group of block has a hashtag in the hashtags in "Required Hashtag" input, it will satisfy that recommendation and that recommendation box will be closed.\\
    \+ When using the hashtags in the different "Required Hashtag" inputs then click "Fix it" at Recommendation sidebar in the higher layer, have to choose each block has the hashtags in each "Required Hashtag" input, it will satisfy those recommendations and those recommendations boxes will be closed if each “Required Hashtag” input are connected a block contains those Required hashtags on “[Component Hashtag](#manage-hashtags)” input.]

5.  When you click outside to not choose the block (named for that port), in the left table of "Configure port" screen, the frame disappears and is replaced by a Port Overview screen that contains the information that: Input port frame (Total number of input ports selected) and and the next frame is Ouput Port frame (Total number of output ports  selected).
6. On the footer of each frame, there is a [See All] button. When you click this button it will list all the block name and the Input / Output name selected on that block. If in all blocks there is no input/output port selected, after clicking on the See all button it will display an icon with the content "No port is defined".
...
1.  Click on each Logit block in the left table of the "Configure port" screen, then tick in the checkboxes shown includes all ports (inputs, outputs of that Logit block), on the right side appears a list that includes all the configurations for the port (chosen input, output or both type) of that Logit block. 
2.  Or At the right side “Port Overview”, you can click to “See All” of 2 options “Input Port” or “Output Port”, it will list the Logit blocks that have an input port or output port of the model.
3.  After clicking on a block and choosing an option, in the left table of the "Configure port" screen, a frame appears with "Name", "Description" input frames, a "Write long Description" button and "Clear Port" button. You have to name that port.
4. When you click outside to not choose the block (named for that port), in the left table of "Configure port" screen, the frame disappears and is replaced by a Port Overview screen that contains the information that: Input port frame (Total number of input ports selected) and and the next frame is Ouput Port frame (Total number of output ports  selected).
6. On the footer of each frame, there is a [See All] button. When you click this button it will list all the block name and the Input / Output name selected on that block. If in all blocks there is no input/output port selected, after clicking on the See all button it will display an icon with the content "No port is defined".
...
#### Definition Basic Of The Unit Block And The logit Flow On The Lowest Layer
...
The “Flow Graph” feature allows you to configure the flow graph by connecting the lower layers. You can set the statement for your flow graph by clicking on the “Flow Graph” button on the sidebar on the right of the workboard. The “Flow Graph” statement is the condition of the shapes on the flow graph that allows those shapes to work and to allow the messages to go into the flow graph.
...
Note: If you drag the valid blocks onto the workboard and after that you change that model of that block become to [a valid model](#definition-basic-of-the-unit-block-and-the-logit-flow-on-the-lowest-layer) and click on the \[Flow Graph\] button, the "Flow Graph" screen will display only the “Start State Shape”.
...
\[Note: The “Start Register” block can affect the result of the statement of the model. If that “Register” block is a “Start Register” block so when you set the hashtags contain prefix “#is\_” or “#now\_” there, those hashtags will get “True” value or active. For more information about the “Start Register” block, see [A Logit Flow On Lokahi](#definition-basic-of-the-unit-block-and-the-logit-flow-on-the-lowest-layer) \]
...
“Spin Layout” feature is an interface that we can create many [Flow graph](#flow-graph-overview) versions of the model to check formal verification. In this layout, we can add many statements for each State on Flow Graph as we want and the Spin server of Lokahi will check the rightness of the statement with the States of the model and return the result.

There are some rules and attentions when using the Spin Layout feature:

A. The "Spin Layout" icon on the toolbar at 3 types of layer.

B. Master Version is the parent of all versions created with the verified model, if you change everything in this model, all statements will be reset.

C. All child versions were created afterverified Master versions. They can’t change the model, add a hashtag or reset statements but they can addmore statements.

D. Each version stores the different versions of the flow graph with the statement of formal verification you have added or edited.

E. When delete or change something inside each version, all the statements and the result of the master will be kept in this new version.

F. You cannot change the statements and the hashtags have been added in the master, just only add new statements in the new version. 

G. You cannot see the [Manage Tags](#manage-hashtags) option when clicking in a State Shape in the Flow Graph

H. You cannot add a hashtag in the flow graph when opening a version.

I. When you choose a statement option of each State during the model that hasn't been completed yet or just have a recommendation on the "Recommendation" sidebar, a pop up with "!" yellow icon appears and shows a content "Your model has not completed. Please finish all the tasks on the recommendation list to set these statements.".

J. When going to the Element layer and Logit layer of the Decision, cannot see any state in the Flow Graph screen of Spin Layout. Because Decision only contains the operation, so it doesn't have any State Shape.

K. When going to a version by clicking on "Open this version" at Version History then click "Back" icon button at toolbar to return to the workplace, it appears a "Lock" icon at "Add Component" sidebar then we cannot edit anything on the workplace even though refresh the workboard page, it still cannot exit the "Lock" state. To disappear that "Lock" state, we have to click the "Spin Layout" button and access the "Spin Layout" screen then click [Back to Master] button.

L. Not allow changing the model (add the hashtag, delete hashtag, edit hashtag) in a version

M. Just allow to add a statement, edit the statement, delete statement in a version.

N. The status of the "Statement" sidebar after adding the statement at [Global Rule and Local Rule](#global-and-local-rule), it still keeps the old result of the old statements even those statements have been deleted in the Global Rule and Local Rule at ["Component Info"](#configure-port) screen and add more the new result of the new statement.

O. You can add many "Add Rule" boxes as you want but if the statement has existed in the statement sidebar, it doesn't add more except that statement is added in Global/Local Rule you can add one more in the Statement sidebar one more time. 

Figure 89

Procedure for creating a version from Master:

1. Click on the "Create new version from master" button.
2. Type in the input "Name" and "Description" textbox.
3. Click the [Save] button.
4. A new version box will appear on the “Version History” sidebar. The version box will display the name as the title (if you create that version has a name) and the date and time and which user created this version in the "Version History" sidebar.
5. You can Edit, Duplicate, or Delete that version by clicking on the menu icon on the right side of each version box.
6. Click on the version box, it expands and shows the "Open this version" link button.
7. When you click the "Open this version" link button of each box in the "Version History" sidebar, it opens that version with differences in the statement and the result of those statements. The sentence "Current version" appears in the box at the "Version History" sidebar and the "Create new version from master" icon button changes to the "Back to Master" icon button to let you know that now you are in this version.

Procedure for creating a statement for the model on Flow Graph at Statement sidebar:

1. Click the "Statement" icon button to open the "Statement" sidebar.
2. The statements after adding the statement at the [Global Rule and Local Rule](#global-and-local-rule) will appear in the “Statement” sidebar.
3. If you want to create a new statement, click on the "Add rules" button next to the title of the “Statement” sidebar.
4. You can only type one statement inside the textbox of the “Add Rules” dialog. To know how to create a statement please go here [Global Rule and Local Rule](#global-and-local-rule).
5. Click the [Save] button then the statement will appear on the “Statement” sidebar and wait for the result from the formal verification server to return.
...
Lokahi supports a visual language that allows users to build out complex requirements by dragging and dropping components on a workboard. The work is almost entirely done with the mouse and requires almost no typing. The visual language uses custom and stock components to represent data and logit flows.
...
[Add Business logit to A Contract Layer (Workboard)](<#add-business-logit-to-a-contract-layer-(workboard)>) [11](<#add-business-logit-to-a-contract-layer-(workboard)>)
...
[Definition Basic Of The Unit Block And The logit Flow On The Lowest Layer](#definition-basic-of-the-unit-block-and-the-logit-flow-on-the-lowest-layer) [96](#definition-basic-of-the-unit-block-and-the-logit-flow-on-the-lowest-layer)
